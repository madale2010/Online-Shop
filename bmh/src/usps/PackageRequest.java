...
public class PackageRequest {
	...
	public static ArrayList<PackageRequest> buildPackageRequest(Cart cart) {
		ArrayList<PackageRequest> requestList = new ArrayList<PackageRequest>();
		
		ArrayList<Product> cartItems = cart.getCartItems();
		for(int i=0; i<cartItems.size();i++){
			PackageRequest packageRequest = new PackageRequest();
			Product product = cartItems.get(i);
			ProductShippingDetails productDetails=product.getShipDetails();
			packageRequest.setID(String.valueOf(i));
			packageRequest.setPounds(String.valueOf(productDetails.getPounds()));
			packageRequest.setOunces(String.valueOf(productDetails.getOunces()));
			packageRequest.setLength(String.valueOf(productDetails.getLength()));
			packageRequest.setHeight(String.valueOf(productDetails.getHeight()));
			packageRequest.setWidth(String.valueOf(productDetails.getWidth()));
			
			packageRequest.setZipOrigination("24210");
			packageRequest.setZipDestination(String.valueOf(cart.getDestinationZip()));
			//Here we need to adjust the rules based on what the user selected.
			//Then populate the data as needed for usps service call
			if(cart.getShippingMethod().equals("")){
				//Priority Express shipping 1-3 days
				packageRequest.setService("PRIORITY");
				//FirstClassMailType is ignored for all mailing request
				//TODO: we need to pull from the product if this is a large or small item.
				//for now we will default to Regular (Options include "Regular","Large")
				packageRequest.setSize("Large");
				//TODO: we need a way to figure out type of container
				//For now we are testing cards so a large ev
				packageRequest.setContainer("Rectangular");
				//This is ignored
				packageRequest.setMachinable("true");
			}
			if(cart.getShippingMethod().equals("")){
				//Priority shipping 2-5 days (UNDER 13oz)
				packageRequest.setService("First-Class Mail");
				//Set the mail type for parcel
				packageRequest.setFirstClassMailType("Parcel");
				//Set the size to large
				packageRequest.setSize("Large");
				//Set the Container 
				packageRequest.setContainer("Rectangular");
				//This is ignored unless mail type is letter or flat 
				packageRequest.setMachinable("true");
			}
			if(cart.getShippingMethod().equals("")){
				//Overnight shipping
				packageRequest.setService("PRIORITY MAIL EXPRESS");
				//This is required when mail type is letter or flat, otherwise ignored.
				packageRequest.setMachinable("true");
			}
			requestList.add(packageRequest);
		}
